...
- T1.7 Behavior beside power. [claimed: Claude, 2026-09-23] One report per run (A3): each round's action, force/prevent thresholds and whether the outcome rests on goals (a coalition smaller than the planner's defectors could force the irreversible label). Acceptance: the three commons designs and one new world use the same report. Symmetry reduction by declared agent types (T8.1) is the scaling path for both queries.
