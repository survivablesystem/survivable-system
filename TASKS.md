...
- E2 Scale by types (T8.1 pulled forward). [claimed: Claude, 2026-09-23; first step: exact symmetry reduction in power queries] Forced by: exact enumeration is exponential in agents; civilizational questions need populations. Agents of one type with identical menus, channels and roles are exchangeable; power and planning over counts instead of identities. Acceptance: exact agreement with individual enumeration where both run; a commons power map at populations beyond the current n <= 4; what symmetry assumes and loses stated.
