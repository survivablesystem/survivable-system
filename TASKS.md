...
- T3.2 Scarce responses and ratio-raising returns. [claimed: Claude, 2026-09-23] Forced by T3.1 findings 1-2. Add a budget that limits building (resources with conversion, T2.2-style, decision first) and a returns form whose gain elasticity can exceed one. Hypothesis: information buys denial only when responses are scarce; lock-in by returns needs elasticity above one. Acceptance: blind versus verified sure prevention counted over a grid where the budget binds; force values by elasticity; contrary counts kept.
