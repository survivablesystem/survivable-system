...
- E2 Scale by types (T8.1 pulled forward). [claimed: Claude, 2026-09-23. Step 1 done: exact symmetry reduction in power queries (`World.types`), commons power exact to n=20 at T=3, power depends on fractions not numbers (`evidence/scale.json`, clean `dc8a981`). Remaining: symmetry in the planner (level-1 responses of exchangeable others; contest branching by counts), types inside composites, and a calibrated approximation only where exact search fails] Forced by: exact enumeration is exponential in agents; civilizational questions need populations. Agents of one type with identical menus, channels and roles are exchangeable; power and planning over counts instead of identities. Acceptance: exact agreement with individual enumeration where both run; a commons power map at populations beyond the current n <= 4; what symmetry assumes and loses stated.
