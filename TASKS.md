...
- T0.1 Reliable evidence and publication. [claimed: Codex, 2026-09-19] Inserted before T1.1: the owner requested improvements after the assessment found confirmation-biased acceptance rules and finite survival mislabeled as an attractor. Acceptance: hypotheses may fail; finite outcomes are named honestly; all CLI modes emit reproducible JSON with source, assumptions, seeds and duration; regression tests and CI pass; publish the project to the owner's GitHub organization. Simplest implementation: retain the planner and dynamics, record existing runs, correct reporting and evidence rules, and make stochastic target order stable. No remote exists yet, so there is nothing to pull before claiming.
