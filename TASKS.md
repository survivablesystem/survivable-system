...
- T3.3 Hidden choice. [claimed: Claude, 2026-09-23] Forced by T3.2 finding 6. Test the general claim: verification buys denial only when the defender's right preventive move depends on the rival's hidden choice. Smallest world change: two capability domains; a strike succeeds in the domain the striker chooses; defense is domain-specific. Acceptance: blind versus verified sure prevention differs in some cells; name them; behavior paired as before. A world where blind equals verified again contradicts the claim.
- A1 case recorded: treaty finding 4. The root opponent model (others repeat their last action) decides whether a trailer anticipates a leader's build. Compare a root-level best-response or sampled-prior model before relying on first-strike timing.
