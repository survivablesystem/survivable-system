...
Below the leaf the planner integrates immediate rewards over successors that share a declared continuation (`World.continuation`, `planning_outcomes`, decision 2026-09-23, E2 step 3): the part of the state every future menu, kernel, choice-relevant observation and goal depends on. Representatives are real successors; payoffs are conditional expectations, exact for additive expected utility. The default declares the whole state; `engine.core.continuation_violations` checks a declaration against the full kernel. Commons declares stock, collapse and last actions (payoffs and wealth are bookkeeping). Commons declares depth choices 1/2/3 and a fixed work cap. Large populations or deeper trees may be unresolved; they cannot be counted as survival or collapse. Exactness means agreement with this finite subjective model and depth, not an optimal institutional design. The investment, nonlinear-risk and hidden-branch diagnostics justify this replacement; see `rediscovery/planner-replacement.md` for measured cost and changed conclusions.
...
- **diff**: any of the above for world A minus world B, or one world under two scenarios. Implemented for one world across a grid (`engine/grid.py`, decision 2026-09-24, E16): `--grid` runs power, externalities or a rule check in every cell of a product of register values, start-state fields and rules, `--draws N` crosses it with seeded draws of the rest of the register; each report is flattened into named measures; per measure, the same in every cell or the keys that change it alone (settings counted); `--compare k` pairs cells differing only in k. "Same in every cell" is relative to the grid. Two different worlds are compared by composing them (E4), not here.
