...
- `tests/test_contract.py` checks every module in `worlds/` against this contract automatically; a new world must pass it.
...
- Declaring a harm as the event that reveals it (a shutdown that failed) instead of the condition behind it (the switch would fail). Declare the condition correction needs, too (a holder nobody can remove). Capture onto an undeclared condition reports as no capture (ai-control 26, standing-army 12).
